# Kontext — eine Struktur

Stand: 2026-09-09, Abend. Dies ist die **eine** Wahrheit für neue Sessions.  
Repo: https://github.com/PromptBrainless/wfrp-async-website · `4a40951` · öffentlich  
Sicherung als Kleist-Bild: [kleist-stand.jpg](../public/images/context/kleist-stand.jpg)  
Presse: [00-KONTEXT-kleist.txt](00-KONTEXT-kleist.txt) · Script: `python3 scripts/kleist-snapshot.py`

---

## Schichten (nicht vermischen)

| Schicht | Sprache | Anfassen |
|---------|---------|----------|
| 1 Buch | `original.md` — tot, gefroren | nur Extraktionsfehler |
| 2 Loop | `angepasst.md` — Karte, Grau, Befehl | ja, präzise |
| 3 Hilfe | `details.md` — SL, Beispiele | ja |
| 4 Leben | Faden, Teaser, SL-Satz | szenisch |
| 5 Kampagne | Drosselau, fünf Plätze | der Mensch setzt |
| 6 Gerät | Engine, Katalog, Seed | hält Zustand, spricht nicht |

## Spiel

WFRP 4e, Ulisses GRW 2. Auflage. Asynchron. Kein Zug — die **Frist** hält den Tisch.  
Spieler: **Absicht** + eigener W100. SL: Leitung, keine Figur, zieht Trigger. Engine: kein Erzähler, kein Zufall ins Leben. Glück nur nach eigenem Wurf. Wahrnehmung **privat** (`privateTo`).

Wörter: [Glossar](../copy/GLOSSAR.md). Sätze: [Spieltexte](../copy/SPIELTEXTE.md). Ort: [Beschreibungen](../copy/BESCHREIBUNGEN.md). Bibliothek: [Beschreibungsregeln V2](07-reikland/drosselau/BESCHREIBUNGSREGELN.md).

## Kampagne 1

Fünf Plätze: **Tick, Track, Truck, Hanni, Nanni**. Ort: **Drosselau** — 5 Viertel, 13 Gassen, 69 Häuser. Graf nicht in der Stadt. Stadt nicht verlassen.

**Erste Stunde, Torstraße, offen.** Arne Helm (Dienst Zollhaus 1, wohnt Nr. 8) prüft. Tick hat sich umgeschaut — nur Tick sieht die Lage. Die Frage hängt: *Namen. Woher. Waffen offen.*

Lagen einer Gasse: offen / grob / teil / voll / scharf / sl. Alle 13 Gassen und alle 69 Häuser handschriftlich. V2-Bibliothek: zuerst die Torstraße vollständig.

## Gerät (Stand `4a40951`)

Lobby: Karte nutzen oder neu (würfeln + Name, oder Erschaffung). Dann nur die Augen des Platzes.  
Türen: **Tisch, Blatt, Journal.** **Pult** nur SL.  
Fuß: offene Frage, Chip dann Zeile dann senden, Mund zum Sagen. Kein Katalog zuerst.  
JSON treibt Katalog. Trigger: SL zieht → Log → Ins Leben. `autoChat` tot. **Tisch = Leben.** Dieselbe Kampagne wie die Erschaffung.

## Nächster Schritt (Mensch, nicht Engine)

Tick antwortet Arne. Die anderen dürfen noch. Die Engine erzählt nicht.

## Nicht

Demo zurück. KI als Erzähler. Zufall ins Leben. Original umschreiben. Stadt verlassen. SL als sechste Figur. Synonyme für Frist, Faden, Blatt, Pult, Absicht. Keime, die von selbst feuern. Stunde/Tag/Woche als Tick der Engine.

### Wege

- [00-WEGE.md](00-WEGE.md) — Abend, Bogen, Buch, Gerät
- [NEUES-FENSTER.md](../NEUES-FENSTER.md)
- [Übergabe](00-UEBERGABE.md)
- [Index](00-INDEX.md)
- [Das Spiel](10-system/00-das-spiel.md)
- [Kampagne 1](10-system/16-kampagne-1.md)
- [Erste Stunde](10-system/18-erste-stunde.md)
- [Drosselau](07-reikland/drosselau/README.md)
