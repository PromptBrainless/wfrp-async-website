# Traglast

Quelle: S. 293

Jeder Gegenstand hat Traglastpunkte (TP), meist 0–3. Maximum ohne Abzug: **STB + WÜB**. Durchschnittlicher Mensch: 6.

Kleine Gegenstände: grob 1 TP pro 200 Münzen. 
Übergroß (4 TP, Fass, Sattel): nur einen, beide Hände. 
Getragen (Rüstung, Kleidung, Schmuck): TP −1, oft 0 am Körper.

Lasttiere ignorieren STB+WÜB; Mensch-Passagier zählt als 10 TP.

## Überladen

Kumulativ mit Rüstungs-Erschöpfung.

| Last | Effekt |
|------|--------|
| bis Maximum | keine Abzüge |
| bis doppelt | −1 Bewegung (min. 3), −10 GW, +1 Erschöpfung durch Reisen |
| bis dreifach | −2 Bewegung (min. 2), −20 GW (min. 10), +2 Erschöpfung durch Reisen |
| mehr als dreifach | keine Bewegung |

Bewegungs-Abzüge sofort, weg wenn Last runter. Erschöpfung durch Reisen: Ende der Tagesreise, weg durch lange Rast.

Beispiele: 0 Messer/Münzen; 1 Schwert/Tasche; 2 Zweihänder/Rucksack; 3 Hellebarde/Fass.

v1-Demo: der Spielercharakter Dolch TP 0, ein Schläger waffenlos TP 0. Traglast greift erst, wenn Inventar wächst.

### Befehl

- kein Katalogeintrag — Inventarfilter. Über Maximum: `sprinten` / `gehen` mit Abzug; über dreifach: Bewegung grau

### Siehe auch

- [Waffen](02-waffen-ruestung.md)
- [Dolch](waffen/dolch.md)
- [Zustände](../05-regeln/05-zustaende.md) (*Erschöpft*)
- [Attribute](../02-charaktere/04-attribute.md)
