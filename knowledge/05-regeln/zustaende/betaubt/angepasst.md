# Betäubt — Projekt

Quelle der Regeln: [original.md](original.md) (S. 167)

Das Buch nicht wiederholen.

Karten mit Handlung: grau, Grund „Betäubt — keine Handlung“. Bewegung halb. Verteidigung bei Vergleich erlaubt, nicht *Sprache (Magick)*.

Redeschwall (Otto) kann diesen Zustand legen. Ende Runde: Engine schlägt Ausdauer vor, SL gibt frei. Letzter Stapel weg → `erschoepft`, falls noch keiner.

## Bogen

`zustaende[]`: `{ id: "betaubt", stapel }`. Kumulativ.

## Grau

Alle H-Karten außer Verteidigung. `mut` sichtbar, solange Stapel > 0.

### Befehl

- `mut` — 1 Stapel weg · Kosten F — [Befehlsregister](../../../10-system/06-befehle.md)

### Siehe auch

- [Original](original.md)
- [Details](details.md)
- [Zustände-Hub](../../05-zustaende.md)
- [Redeschwall](../../../04-faehigkeiten-talente/talente/redeschwall/angepasst.md)
- [Erschöpft](../erschoepft/angepasst.md)
- [Ausdauer](../../../04-faehigkeiten-talente/faehigkeiten/ausdauer/angepasst.md)
