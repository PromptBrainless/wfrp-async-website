...
Die digitale Form dieses Ordners ist die Bibliothek der Kampagne. Eine Szene ist die Bühne: wo die SC stehen und was um sie herum geschieht (S. 261). Die Bühne **zieht** aus den Pools. Sie **besitzt** die Einträge nicht. ein Händler Tuchhändler ist ein Händler, ob er hinter den Ballen am Altquartier-Basar steht oder später in einer Stube denselben Preis verteidigt. Würde jede Karte ihren eigenen ein Händler tragen, müsste der SL ihn neu erfinden, sobald der Spielercharakter den Stand verlässt. Wunden, Geld, Haltung, Flags — alles ginge verloren oder würde sich verdoppeln. Das ist kein WFRP-Tisch, das ist ein Videospiel-Spawn.
...
- **Beutetabellen.** Was unter Ottos Tuch oder in Kurts Taschen liegen könnte. Der Wurf ist ein Vorschlag an den SL, kein Zugriff aufs Inventar.
...
Das spart Vorbereitung und hält die Welt zusammen. Der SL legt ein Händler einmal an, taggt ihn als Händler und Bürger, schreibt sich einen Satz für den eigenen Blick — und kann ihn am Basar, in der Stube, auf der Hauptstraße ziehen, ohne den Menschen neu zu bauen. Dieselbe Logik gilt für ein Schläger, für die Wache, für die Menge-Tabelle, für das, was unter dem Tuch liegt. Wiederverwendung ist hier keine Optimierung der Engine. Sie ist die Art, wie ein SL einen Menschen erinnert.

macht das greifbar. Kampagne „Der Ballen“, zwei Szenen: Basar-Altquartier und die Gasse hinter dem Basar. der Spielercharakter ist die SC, einmal erschaffen, persistent. ein Händler steht am Stand. ein Schläger wartet in der Gasse. Die Wache gähnt am Pfeiler, ist in der Bibliothek, ist auf dem Kampagnenbogen — und ist trotzdem erst dann *in* der Szene, wenn der SL sie zieht. Keiner von ihnen ist Eigentum der JPEG-Karte.
...
`NpcPoolEntry` trägt vier Felder. `id` ist der Schlüssel im Pool (`otto`, `kurt`, `wache`). `characterId` zeigt auf denselben Schlüssel in `campaign.characters` — fallen die beiden zusammen, sie dürfen später auseinanderlaufen, falls ein Pool-Eintrag auf einen bereits existierenden Bogen zeigt. `tags[]` sind Suchworte für den SL, keine Regeln: ein Händler trägt `haendler`, `basar`, `buerger`; ein Schläger `raufbold`, `gasse`, `gesetzlose`; die Wache `ordnung`, `basar`. Tags filtern die Schublade. Sie spawnen niemanden. `blurb` ist ein Satz **für den SL**. ein Händler: „Hält den Preis. Feilschen ist seine Waffe.“ ein Schläger: „Will den Ballen. Offene Hände, noch keine Klinge.“ Die Wache: „Gähnt, hört selten hin, kommt aber, wenn es laut wird.“ Dieser Satz erscheint nicht von selbst auf der Spieler-Journalkarte. Der Spieler sieht, was der SL freigibt, in der Stimme der Welt, nicht die Regieanweisung.

Der Bogen selbst — Attribute, Fähigkeiten, Talente, LP, Geld, Inventar, Flags, Haltung, Portrait — liegt in `campaign.characters`. NSCs nutzen dasselbe Gerüst wie SC (`kind: "npc"`), plus Haltung. Ohne diesen Bogen gibt es keine legale Aktion und keinen legalen Vergleich. Feilschen gegen ein Händler ist ein Wurf gegen Ottos Feilschen, nicht gegen eine Atmosphäre namens „Händler am Stand“.
